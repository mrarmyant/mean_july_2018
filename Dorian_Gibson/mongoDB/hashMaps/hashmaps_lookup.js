...
function hashFind (map, key){
    var hashedKey = key.hashCode();
    var idx = mod(hashedKey, map.length);
    if(map[idx]){
        for(var i=0; i<map[idx].length; i++){
            if(map[idx][i][0] == key){
                console.log(map[idx][i][1]);
                return;
            }
        }
    }
...
hashFind(hashMap,'car');
hashFind(hashMap,'laptop');
hashFind(hashMap,'truck');
